...
class TootsTest(unittest.TestCase):
    ...
    def setUp(self):
        self.test_toots_file = os.path.join(toots.app.root_path, 'fixtures',
                                            'toots_small.json')
        self.test_authors_file = os.path.join(toots.app.root_path, 'fixtures',
                                              'authors.json')
    ...
    def test_make_structured_data(self):
        ...
        self.assertEqual(
            len(structured_data[0]['children'][0].get('children')), 1)
        self.assertEqual(
            structured_data[0]['children'][1]['children'][0]['children'][
                0].get('text'), 'zoot choot!')
        self.assertEqual(
            structured_data[0]['children'][1]['children'][0]['children'][
                0].get('parent_id'), 97)
    ...
    def test_toot_threads_by_author(self):
        ...
        toots = get_toots_by_author(data=toots_data,
                                    author_id_to_filter=author_id_to_filter)
